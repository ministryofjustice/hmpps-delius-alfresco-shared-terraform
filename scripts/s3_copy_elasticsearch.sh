...
# dest s3 bucket
DEST_S3_BUCKET="${STAGING_BUCKET_NAME}"
...
if [ ${RUN_MODE} = true ]
then
  echo "Run mode set to ${RUN_MODE}, --only-show-errors flag set. Only errors will be shown and no progess updates"
  get_creds_aws
  aws s3 rm s3://${DEST_S3_BUCKET} --recursive
  exit_on_error $? !!

  get_creds_aws
  aws s3 sync --only-show-errors s3://${SRC_S3_BUCKET}/${SRC_BUCKET_PATH}/elasticsearch s3://${DEST_S3_BUCKET}/elasticsearch/
  exit_on_error $? !!
  ...
  get_creds_aws
  aws s3 sync s3://${SRC_S3_BUCKET}/${SRC_BUCKET_PATH}/elasticsearch s3://${DEST_S3_BUCKET}/elasticsearch --dryrun
  exit_on_error $? !!
