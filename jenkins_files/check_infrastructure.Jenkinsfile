...
pipeline {
    ...
    stages {
        ...
        stage('setup') {
            steps {
                ...
                dir( project.alfresco ) {
                  git url: 'git@github.com:ministryofjustice/' + project.alfresco, branch: 'issue_150_rehearsal_fixes', credentialsId: 'f44bc5f1-30bd-4ab9-ad61-cc32caf1562a'
                }
        ...
        stage('Delius | Alfresco ES Migration') {
            when {
                expression { "${environment_name}" == "delius-prod"}
            }
            steps { 
                script { do_terraform(environment_name, project.alfresco, 'elk-migration')}
            }
        }
